...
1. 遍历 .pcap 文件
    遍历指定源目录中的所有子目录，找到 .pcap 文件。
2. 特征提取
    使用 tshark 命令从 .pcap 文件中提取网络流量的时间戳、协议、包长度、MAC 地址、IP 地址、端口号、TCP/UDP 标志等特征。
    将提取结果保存为对应的 .csv 文件。
3. 数据包方向判断
    根据设备的 MAC 地址映射字典，判断数据包的方向：
        如果数据包的源 MAC 地址匹配设备，则方向为 1（设备发送）。
        如果目标 MAC 地址匹配设备，则方向为 -1（设备接收）。
4. 保存处理结果
    在生成的 .csv 文件中新增 direction 列，记录每个数据包的方向信息。
    保存最终的 .csv 文件，替换临时文件。
5. 进度输出
    在终端打印处理的文件路径、生成的命令及处理进度。
"""
...
def extract_features_from_pcap(source_root, destination_root, device_mac_mapping):
    """
    从 pcap 文件中提取特征信息，并将结果保存为 CSV 文件。
...
def main():
    # 定义源目录和目标目录`
    source_directory = "/home/hyj/deviceIdentification/dataset/UNW2016/4_suitableDir"
    destination_directory = "/home/hyj/deviceIdentification/dataset/UNW2016/5_csv"
